...
export class GoogleCalendarService {
  ...
  static async syncAppointmentEvent(data: {
    ...
    startTime: Date;
    endTime: Date;
  }): Promise<{ success: boolean; googleEventId?: string; message?: string }> {
    try {
      const conn = await GoogleCalendarConnection.findOne({ userId: data.userId });
      ...
      if (env.GOOGLE_CLIENT_ID === 'mock_google_client_id' || env.GOOGLE_CLIENT_ID === 'dev_google_client_id') {
